...
# Smaller than iteration-1's 85MB: iteration-2 cells carry up to 1000
# users (after the adapter subsample) and |V|=500 top-50 predict lists, so
# an 85MB target overshoots the ~100MB file convention (measured: parts up
# to 121MB; single |V|=500 catalogs serialize to ~52MB blobs).  At 40MB,
# parts stay well under 100MB (one big catalog per part at most).
PART_SIZE_MB = 40
